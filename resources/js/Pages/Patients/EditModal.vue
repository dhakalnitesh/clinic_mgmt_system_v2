    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl w-full max-w-3xl shadow-2xl overflow-y-auto max-h-[90vh]">
      ...
      <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200 dark:border-gray-700">
        <h3 class="text-xl font-bold text-gray-900 dark:text-gray-100">
          <i class="fas fa-user-edit text-teal-600 mr-2"></i>Edit Patient
        </h3>
...
const form = useForm({
  ...
  address1: props.patient?.address1 ?? '',
  notes: props.patient?.notes ?? '',
  citizenship_type: props.patient?.citizenship_type ?? '',
  province_id: props.patient?.province_id ?? '',
  district_id: props.patient?.district_id ?? '',
  municipal_id: props.patient?.municipal_id ?? '',
  certificate_number: props.patient?.certificate_number ?? '',
})
