  ...
  <AuthenticatedLayout title="Purchase Orders">
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm mb-4 p-4 flex flex-wrap items-end gap-3">
      <div class="relative flex-1 min-w-52">
        <label for="filter-po-search" class="text-xs font-medium text-slate-500 mb-1 block">Search</label>
        <MagnifyingGlassIcon class="absolute left-3 top-9 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
        <input id="filter-po-search" v-model="filters.search" @input="debounceSearch" type="text"
               placeholder="Search PO number or supplier…"
               class="form-input pl-9" />
      </div>
      <div class="flex flex-col gap-1">
        <label for="filter-po-supplier" class="text-xs font-medium text-slate-500">Supplier</label>
        <select id="filter-po-supplier" v-model="filters.supplier" @change="applyFilters" class="form-select w-48">
