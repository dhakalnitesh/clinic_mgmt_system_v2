<template>
  <AuthenticatedLayout title="Add Drug Interaction">
    ...
    <div class="mb-6 flex items-center gap-3">
      ...
      <div>
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Add Drug Interaction</h1>
        <p class="text-sm text-slate-500 mt-0.5">Define a new interaction rule between two generic drugs</p>
      </div>
    </div>
    ...
    <form @submit.prevent="submit" class="max-w-2xl space-y-6">
      ...
      <div class="flex items-center gap-3 pt-2">
        <button type="submit" :disabled="form.processing"
          ...
          <span v-else>Create Interaction</span>
        </button>
        <Link :href="route('pharmacy.drug-interactions.index')"
...
const submit = () => {
  form.post(route('pharmacy.drug-interactions.store'))
}
</script>
