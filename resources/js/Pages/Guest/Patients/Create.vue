  ...
  <AuthenticatedLayout>
    ...
    <div class="max-w-3xl mx-auto">
      ...
      <div v-if="alreadyCreated" class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-8 text-center">
        <div class="text-5xl mb-4 text-teal-500">
          <i class="fas fa-check-circle"></i>
        </div>
        <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2">Patient Registered!</h2>
        <p class="text-gray-500 dark:text-gray-400 mb-6">
          You have already registered a patient. You can now book an appointment.
        </p>
        <Link href="/guest/appointments/create"
          class="inline-block px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-medium transition">
      ...
      <div v-else class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
        <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <h3 class="text-xl font-bold text-gray-900 dark:text-gray-100">
            <i class="fas fa-user-plus text-teal-600 mr-2"></i>Register Patient
          </h3>
        </div>
...
const submit = () => {
  form.post(route('guest.patients.store'), {
    preserveScroll: true,
    onSuccess: () => {
      form.reset()
    },
